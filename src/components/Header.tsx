...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full transition-all">
      ...
      <div className="bg-[#121214] text-[#f8f7f4] text-[10px] sm:text-[11px] py-1.5 sm:py-2 px-3 sm:px-4 text-center font-mono-code uppercase tracking-wider flex items-center justify-center gap-1.5 sm:gap-2 overflow-hidden border-b border-red-900/40">
      ...
      <div className="bg-white/95 backdrop-blur-xl border-b border-black/10 shadow-xs">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-18 lg:h-20 gap-2 sm:gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 sm:gap-3 text-left group focus:outline-hidden cursor-pointer shrink-0 min-w-0"
              title="Cabai Enterprise - 3D Printing & Maker Studio"
            >
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-[#18181b] border-2 border-red-600/60 overflow-hidden shadow-xs group-hover:scale-105 group-hover:border-red-500 transition-all flex items-center justify-center p-0.5 shrink-0">
                <img 
              ...
              <div className="flex flex-col min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-[8px] sm:text-[9px] px-1 py-0.2 rounded-xs sm:rounded-sm bg-red-100 text-[#af101a] font-mono-code font-bold">
                    Penang
                  </span>
                </div>
              </div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-3 py-1.5 xl:px-3.5 xl:py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer flex items-center gap-1.5 select-none ${
                      isActive 
            ...
            {/* RIGHT ACTIONS & CONTROLS (RESPONSIVE FOR ALL SCREEN SIZES) */}
            <div className="flex items-center gap-1 sm:gap-2 lg:gap-2.5 shrink-0">

              {/* Shopping Cart Trigger */}
              <button
                onClick={() => setIsCartOpen(true)}
                className="relative p-2 sm:px-3 sm:py-2 rounded-xl bg-black/5 hover:bg-[#af101a] text-[#18181b] hover:text-white transition-all duration-200 group cursor-pointer flex items-center gap-1.5 sm:gap-2 border border-black/5 shadow-2xs min-h-[40px] min-w-[40px] justify-center"
              ...
              {currentUser ? (
                <button
                  ...
                  className={`flex items-center gap-1.5 sm:gap-2 py-1.5 px-2 sm:px-3 rounded-xl transition-all font-bold text-xs border shadow-2xs cursor-pointer min-h-[40px] ${
                ...
                <div className="flex items-center gap-1 sm:gap-1.5">
                  <button
                    onClick={() => setIsAuthModalOpen(true)}
                    className="hidden md:flex items-center gap-1.5 py-2 px-2.5 sm:px-3 rounded-xl bg-white hover:bg-black/5 text-[#18181b] transition-all font-bold text-xs uppercase tracking-wider border border-black/10 cursor-pointer shadow-2xs min-h-[40px]"
                    title="Sign In"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 sm:gap-1.5 py-1.5 px-2.5 sm:py-2 sm:px-3.5 rounded-xl bg-[#af101a] hover:bg-[#8e0c15] active:scale-95 text-white transition-all font-bold text-xs uppercase tracking-wider cursor-pointer shadow-sm shadow-red-950/20 min-h-[40px]"
              ...
              {/* Boss Admin Console Link (Desktop/Tablet) */}
              <a
                href="https://admin-beta-pink-11.vercel.app/"
                target="_blank"
                rel="noopener noreferrer"
                className="hidden xl:flex items-center gap-1.5 px-2.5 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-all border bg-[#18181b] text-white border-black/20 hover:bg-[#af101a] hover:border-[#af101a] shadow-xs cursor-pointer min-h-[40px]"
      ...
      {/* MOBILE / TABLET DRAWER (FULL-SCREEN RESPONSIVE OVERLAY) */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[calc(theme(spacing.16)+30px)] sm:top-[calc(theme(spacing.18)+34px)] bottom-0 z-50 bg-black/50 backdrop-blur-xs flex flex-col justify-start animate-fadeIn">
          <div 
          ...
          >
            ...
            <div className="pt-2 border-t border-gray-100">
              <a
                ...
                className="w-full px-3.5 py-3 rounded-xl text-xs font-bold bg-[#18181b] hover:bg-[#af101a] text-white flex items-center justify-between transition-colors shadow-xs min-h-[44px]"
              >
